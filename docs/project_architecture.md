# Kaggle Competition Development Platform - 完全アーキテクチャ設計書
...
│    ▼                                                             │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │  1. 知識・タスクデータベース (KaggleBase - Obsidian + Git) │ │
│  │                                                             │ │
│  │  [Inbox] → [Zettelkasten] → [GTD Tasks]                    │ │
│  │              ↑                   │                          │ │
│  │              └── [References] ───┘                          │ │
│  └───────────────────────────────────────────────────────────┘ │
│                           │                                      │
...
│  │  2. JSON形式タスク管理                                     │ │
│  │                                                             │ │
│  │  [plan.json] → [Pending] → [In Progress] → [Completed]    │ │
│  └───────────────────────────────────────────────────────────┘ │
│                           │                                      │
│                           ▼                                      │
│  ┌───────────────────────────────────────────────────────────┐ │
...
│  │                             ↓                               │ │
│  │          [Docs Manager] → [Version Controller]             │ │
│  └───────────────────────────────────────────────────────────┘ │
│             │                        │                          │
│             ▼                        ▼                          │
│  ┌─────────────────┐    ┌─────────────────────────┐            │
...
│  │  5. MLOpsパイプライン (知人担当)                           │ │
│  │                                                             │ │
│  │  [Data Pipeline] → [Model Training] → [Deploy]             │ │
│  │                                         ↓                   │ │
│  │                                    [Monitoring]             │ │
│  │                                         │                   │ │
...
データフロー:
  開発者 → Inbox → Zettelkasten/GTD → plan.json → エージェント → 
  実験/結果 → MLOps → モニタリング → フィードバック → Zettelkasten
```
...
#### 概要
ObsidianのGTDタスクをJSON形式に変換し、マルチエージェントシステムに引き渡す仕組み。`plan-template/`で既に実装されているスキーマを採用します。
...
def generate_plan_json(task_dir, output_path):
    ...
    print(f"✓ plan.jsonを生成しました: {output_path}")
...
graph TB
    ...
    Convert --> PlanJSON[5. plan.json生成]
...
3. task_converter.pyを実行してplan.jsonを更新

## 午後: 実験実行
4. Cursorでplan.jsonを開く
...
│   └── test_utils.py
│
├── plan-template/                    # タスク管理テンプレート（既存）
│   ├── .cursor/
│   │   └── plan.json                # plan.jsonの雛形
│   ├── docs/
│   │   └── ROADMAP.md
│   └── scripts/
│       └── generate_roadmap.py      # plan.json → Markdown変換
│
├── mcp_setup/                        # MCP設定（将来）
│
├── .gitignore                        # Git除外設定
...
### Q2: plan.jsonとGTDタスクの関係は？
**A**: GTDタスク（Markdown）が人間向け、plan.jsonがAIエージェント向け。task_converter.pyで相互変換可能。
