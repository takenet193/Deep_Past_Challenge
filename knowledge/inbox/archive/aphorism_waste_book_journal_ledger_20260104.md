...
promoted_to: wastebook_journal_ledger_20260104235040
promoted_at: 2026-01-04
---

# アフォリズム（孫引き）: Merchants have their waste book...
...
- “waste book（wastebook）→ journal → ledger（double-entry）” という **三段階の記録プロセス**が、学者（learned）にも模倣に値するとされている点が重要
- Sudelbuch / Klitterbuch という語の揺れ（綴り）も含め、原典確認が必要

## TODO（後で整える）

- 孫引き元（自分が読んだテキスト）と一次情報（出典・ページ）を確定する
- 可能なら日本語訳を併記（quote_translation）
- “帳簿/SSOT/コンテクストエンジニアリング” の議論に接続して、zettelkasten/permanent へ昇格検討
