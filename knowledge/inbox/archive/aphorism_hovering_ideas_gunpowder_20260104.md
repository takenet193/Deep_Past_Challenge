---
created: 2026-01-04
type: aphorism_inbox
status: archived
tags: [aphorism, quote]
promoted_to: hovering_ideas_gunpowder_20260104235000
promoted_at: 2026-01-04
---

# アフォリズム: Oh how many ideas aren’t hovering dispersed...

## 引用

> “Oh how many ideas aren’t hovering dispersed in my head! Quite a few pairs among those could provoke the greatest discovery if only they came together. But isolated from one another they lie, just like the sulphur from the city of Goslar lies isolated from East Indian nitre and from Oaksfield coal dust when jointly they could produce gunpowder!”

## メモ

- 「頭の中に分散して漂うアイデア同士が結びつけば、最大の発見を生む」という比喩
- 分散→結合→創発、という発想はZettelkasten（リンクによる組み合わせ）と相性が良い

## TODO（後で整える）

- 出典（著者/作品名/年/章/ページ）を確定
- 可能なら日本語訳を併記
- 関連ノート（ZKの設計思想・コンテキストエンジニアリング）へリンク









